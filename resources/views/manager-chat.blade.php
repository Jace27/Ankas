...
@section('content')
    <div class="b-white p-1 d-flex justify-content-between">
        <h1>Общение с клиентом</h1>
        <button class="btn btn-secondary mt-2 mb-2" onclick="window.location.assign('{{ route('chats') }}')">Назад</button>
    </div>
    <div class="b-white" style="position: relative; height: 700px">
        <div class="chat">
            ...
            $.ajax({
                ...
                success: function(data, status, xhr){
                    ...
                    if (xhr.responseJSON != null){
                       ...
                       data = xhr.responseJSON; data.messages.forEach(function(item, index, array){
                            let viewed = '';
            ...
            $.ajax({
                ...
                success: function(data, status, xhr){
                    ...
                    if (xhr.responseJSON != null && data.status == 'success'){
                        ...
                        display_message('Просмотр сообщений', 'Ошибка!<br>'+data.message);
                    }
                },
                error: function(xhr){
                    console.log(xhr);
